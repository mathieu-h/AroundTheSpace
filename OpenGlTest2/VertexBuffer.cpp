...
VertexBuffer::VertexBuffer(const GLvoid *data, GLsizei size, GLenum mode, GLsizei count, GLsizei stride, ShaderInterface* shader, ShaderData* shaderData, GLvoid* positionOffset, GLvoid* normalOffset, GLvoid* textureCoordOffset, std::vector<GLuint> indices, GLuint amount, ShaderInterface* _shaderdepth) : _mode(mode), _count(count), _stride(stride), _shader(shader), _shaderData(shaderData), _positionOffset(positionOffset), _normalOffset(normalOffset), _textureCoordOffset(textureCoordOffset)
{
	_vexterArrayID = -1;
	...
	if (_shaderdepth->get_aPositionVertex() != -1) {
		glEnableVertexAttribArray(_shaderdepth->get_aPositionVertex());
		glVertexAttribPointer(_shaderdepth->get_aPositionVertex(), 3, GL_FLOAT, GL_FALSE, _stride, _positionOffset);
	}

	if (_shaderdepth->get_aPositionNormals() != -1) {
		glEnableVertexAttribArray(_shaderdepth->get_aPositionNormals());
		glVertexAttribPointer(_shaderdepth->get_aPositionNormals(), 3, GL_FLOAT, GL_FALSE, _stride, _normalOffset);
	}

	if (_shaderdepth->get_aTextCoords() != -1) {
		glEnableVertexAttribArray(_shaderdepth->get_aTextCoords());
		glVertexAttribPointer(_shaderdepth->get_aTextCoords(), 2, GL_FLOAT, GL_FALSE, _stride, _textureCoordOffset);
	}
...
VertexBuffer::VertexBuffer(std::vector<VertexDataPNT> data, GLsizei size, GLenum mode, GLsizei count, GLsizei stride, ShaderInterface* shader, ShaderData* shaderData, GLvoid* positionOffset, GLvoid* normalOffset, GLvoid* textureCoordOffset, std::vector<GLuint> indices, Vector3 _originPos, float _timeOffset, float _speed) : _mode(mode), _count(count), _stride(stride), _shader(shader), _shaderData(shaderData), _positionOffset(positionOffset), _normalOffset(normalOffset), _textureCoordOffset(textureCoordOffset), originPos(_originPos), speed(_speed), timeOffset(_timeOffset)
{
	_vexterArrayID = -1;
